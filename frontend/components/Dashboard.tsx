...
export default function Dashboard() {
  ...
  const [userPhoto, setUserPhoto] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem(`user_photo_${userId}`) || clerkUser?.imageUrl || "";
    }
  ...
  useEffect(() => {
    if (userPhoto && userId) {
      localStorage.setItem(`user_photo_${userId}`, userPhoto);
    }
  }, [userPhoto, userId]);
            ...
            {!loading && selectedReport && (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {selectedReport.param_interpretation && Object.entries(selectedReport.param_interpretation).map(([key, data]: [string, any]) => {
                    const isAbnormal = data.status !== "normal";
                    return (
                      <motion.div
                        key={key}
                        whileHover={{ y: -5 }}
                        className={`p-5 rounded-xl border transition-all duration-300 ${isAbnormal ? "bg-white/10 border-white/20" : "bg-white/5 border-white/5"}`}
                      >
                      ...
                      <div className="text-sm text-zinc-300 leading-relaxed">
                        {Array.isArray(selectedReport.risk_rationale) ? (
                          <ul className="space-y-2">
                            {selectedReport.risk_rationale.map((reason: string, idx: number) => (
